...
class ComicGenerator:
    ...
    def generate_comic_strip(self, article_title: str, article_content: str, 
        ...
        """
        if not self.model:
            logger.error("Gemini model not available for comic generation")
            return self._create_placeholder_comic(article_title)
        
        try:
            # Generate comic dialogue
            dialogue = self._generate_comic_dialogue(article_title, article_content)
            if not dialogue:
                return self._create_placeholder_comic(article_title)
    ...
    def _generate_comic_dialogue(self, title: str, content: str) -> Optional[Dict]:
