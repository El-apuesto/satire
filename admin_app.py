...
def debug():
    ...
    try:
        ...
        # Test Groq import
        try:
            # Test if we can make HTTP requests to Groq API
            import requests
            response = requests.get("https://api.groq.com/openai/v1/models", 
                                headers={"Authorization": f"Bearer {Config.GROQ_API_KEY}"})
            debug_info["groq_import"] = response.status_code == 200
            
            # Test Groq client creation
            if Config.GROQ_API_KEY:
                debug_info["groq_client"] = True
            else:
                debug_info["groq_client"] = False
                debug_info["groq_error"] = "No API key configured"
        except Exception as e:
